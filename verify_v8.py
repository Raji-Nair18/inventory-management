...
with app.app_context():
    ...
    user = User.query.filter_by(username='test_owner').first()
    if not user:
        user = User(username='test_owner', role='shop_owner')
        db.session.add(user)
        db.session.commit()
    ...
    product = Product.query.filter_by(sku='09', shop_id=shop.id).first()
    if not product:
        product = Product(
            sku='09', name='abc', category='General',
    ...
    if not expired_entry:
        expired_entry = ExpiredProduct(
            ...
            name='abc', sku='09', category='General',
    ...
    with open('verify_output.txt', 'w') as f:
        f.write(f"Initial state: Product archived={product.is_archived}, ExpiredProduct entries={ExpiredProduct.query.filter_by(product_id=product.id).count()}\n")

        # Simulate restocking (what owner did)
        product.is_archived = False
        product.expiry_date = datetime.now().date() + timedelta(days=30)
        product.stock_quantity = 38
        db.session.commit()
        
        # The get_products cleanup should run when we fetch products
        ...
        f.write(f"After restock & cleanup: Product archived={product.is_archived}, ExpiredProduct entries={ExpiredProduct.query.filter_by(product_id=product.id).count()}\n")
